...
@router.post("/companies/{company_id}/platforms/{platform_id}/text/{field_id}/delete")
def platform_text_delete(company_id: int, platform_id: int, field_id: int):
    with db.connect() as conn:
        conn.execute(
            update(platform_text_fields)
            .where(and_(
...
@router.post("/companies/{company_id}/platforms/{platform_id}/word/upload")
async def platform_word_upload(request: Request, company_id: int, platform_id: int, file: UploadFile = File(...)):
    ...
    # 存本地：uploads/{company_id}/platforms/{platform_id}/word/{uuid}_{name}
    ...
    with db.connect() as conn:
        res = conn.execute(
            insert(documents).values(
                company_id=company_id,
                uploaded_by=user_id,          # ✅补上
                group_key=guess_group_key(),
                file_type="platform_word",
                category="platform",
                title="Platform Word",
                ...
                mime_type=mime,
                file_size=size,
                file_sha256=None,             # ✅如果你表允许 NULL 就不写；不允许就先算一个
                is_deleted=0,                 # ✅如果你表没默认值，也补上
                created_at=datetime.utcnow(),
            )
        )
        doc_id = res.lastrowid

        # 2) 写 platform_documents（doc_role='word'）
        conn.execute(
